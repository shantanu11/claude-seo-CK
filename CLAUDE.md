...
claude-seo/
  ...
  scripts/                         # Python execution scripts (21 tracked + 2 dev-only)
    ...
    gsc_inspect.py               # URL Inspection (single + batch + concurrent + direct URLs)
    indexing_notify.py           # Indexing API v3 (URL_UPDATED/URL_DELETED)
    ga4_report.py                # GA4 organic traffic reports
    google_report.py             # PDF/DOCX/XLSX/HTML report generator (WeasyPrint + matplotlib + python-docx)
...
- **Dependencies**: `matplotlib>=3.8.0` (charts) + `weasyprint>=61.0` (HTML-to-PDF) + `python-docx>=1.1.0` (DOCX) + `openpyxl>=3.1.0` (XLSX), all in `requirements.txt`
- **Formats**: PDF (default), DOCX, XLSX, HTML. Use `--format docx` for Word, `--format all` for all formats
- **PDF format**: A4 via WeasyPrint + matplotlib charts at 200 DPI
- **DOCX format**: Professional Word document via python-docx with branded Calibri styles, navy headers, embedded chart images, formatted tables
...
- **Cross-skill enforcement**: After completing ANY analysis command (audit, page, technical, content, schema, geo, local, maps), offer: "Generate a report? Use `/seo google report` (formats: pdf, docx, xlsx)"
